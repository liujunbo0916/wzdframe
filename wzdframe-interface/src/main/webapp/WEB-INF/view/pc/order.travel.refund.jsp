            ...
            <c:forEach items="${dataModel}" var="item" varStatus="status">
	                ...
	                <div class="col-md-3 content-one">
	                    <div class="row-one" style="margin-bottom:20px;">
	                        <span class="item-one">
	                            <span class="label-name">退款单号:</span>
	                    ...
	                    <div class="row-three">
	                         出行日期：${item.departure_date}
	                    </div>
	                </div>
	                <div class="col-md-1 content-two"> ${item.to_name}
	                      <c:if test="${empty item.to_type ||  item.to_type eq 1}">
	                          （成人）
	                      </c:if>
	                          <c:if test="${not empty item.to_type &&  item.to_type eq 0}">
	                         （儿童）
	                      </c:if>
	                </div>

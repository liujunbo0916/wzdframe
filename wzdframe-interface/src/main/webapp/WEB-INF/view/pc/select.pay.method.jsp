...
<div class="body">
        ...
        <div class=" good-info">
            ...
            <c:forEach items="${dataModel}" var="item">
	            <div class="row good-content">
	                <div class="col-md-8 col item-left">
	                    ${item.goods_name} <span style="font-size: 12px;">
	                    <c:if test="${not empty item.goods_attr && item.goods_attr ne '-1'}">
	                       ${item.goods_attr}
	                    </c:if>
	                    </span>
	                </div>
	                <div class="col-md-4 col item-right">x${item.goods_count}</div>
	            </div>
            </c:forEach>
            <div class="row good-content-two">
                ...
                <div class="col-md-4 col item-right">应付金额：<span style="    color: #fc9313;">¥${orderInfo.pay_by_money}</span></div>
            </div>
        </div>
    ...
    <div class="modal fade" id="weixinModal" tabindex="-1" role="dialog"  aria-labelledby="weixinModalTitle"  aria-hidden="true">
        <div class="modal-dialog weixin-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header weixin-header">
                ...
                <div class="modal-body weixin-body">
                    ...
                    <img style="border: 1px solid #D5D5D5;" id="zhifuQR" src="/img/duyun/pc/home/qhyscercode.jpg" class="weixin-scanner" alt="">
                </div>
            </div>
        </div>
    </div>
...
<script type="text/javascript">
  //计时器
   ...
   //支付宝1 微信2
   var submitInfo = {
 		  type:2,//支付方式   2是商品支付  1是票务支付
 		  payType:1, //支付方式   2是微信支付  1是支付宝支付
 		  orderId:'${orderInfo.order_id}',
 		  money:'${orderInfo.pay_by_money}'
   }
  $(function(){
	 ...
	 $("#payNow").click(function(){
		 if(submitInfo.payType == 1){
			...
			/*  window.open("/pay/getPayConfig?paramStr="+Base64.encode(JSON.stringify(submitInfo)),'_blank');
			 window.close(); */
		 }else{
   ...
   function requestOrderPayStatus(orderStatus){
		  Ajax.request("/pay/scanSuccess",{"data":{'orderId':'${orderInfo.order_id}',"type":"product"},"success":function(data){
			  if(data.code == 200){
				  var data = data.data;
				  if(data.pay_status == 1){
					  ...
					  setTimeout(function(){
						  var orderInfo = {
								  type:'product',
								  id:data.id,
								  to_contact_mobile:data.contact_phone,

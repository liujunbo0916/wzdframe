...
<div class="row service-nav">
    ...
    <div class="col-md-6">
        <div class="row service-nav-right">
          <div style="cursor: pointer;" onclick="window.location.href='/hotel/list';" class="col-md-2 service-0">
                酒店预订
            </div>
            <div style="cursor: pointer;" class="col-md-2 service-1"  onclick="window.location.href='/ticket/list'">
                门票预订
            </div>
            <div style="cursor: pointer;" class="col-md-2 service-2 active" onclick="window.location.href='/travel/route'">
                旅游路线
            </div>
        </div>
    </div>
...
<div class=" travel-content">
    <div >
        <div class="ticket-header">
            ...
            <span class="service-color-text">旅游路线</span>
        </div>
        <div class="row comprehensive-content ">
        <div class="col-md-9 content-left">
            <div class="row sale-row">
                <div class="comprehensive-sequence <c:if test='${empty pd.order_type || pd.order_type eq "multiple"}'>active</c:if>" data-code="multiple">
                    综合排序
                </div>
                <div class="route-sale <c:if test='${pd.order_type eq "sale_num"}'>comprehensive-sequence active</c:if>" data-code="sale_num">
                    销量优先
                </div>
                <div class="route-price <c:if test='${pd.order_type eq "price_order"}'>comprehensive-sequence active</c:if>" data-code="price_order">
                    价格
                </div>
            </div>
            ...
            <div class="row router-list">
	            <c:forEach items="${page.resultPd}" begin="${ll*3}" end="${ll*3+2}" var="item" varStatus="status">
		                <div  onclick="goRouteDetail('${item.grouptour_id}');" class="col-md-4 router-list-item <c:if test='${status.index eq (ll*3+2)}'>last</c:if>">
		                    <div class="image-hover">
		                        <img style="width:270px;" src="${SETTINGPD.IMAGEPATH}${item.grouptour_img}" onError="javascript:this.src='/img/no-img/no-img.jpg';" alt="">
		                        <div class="list-hover" style="height:62px;overflow: hidden;">
		                            <p class="hover-header">${item.grouptour_name}</p>
		                            <p class="hover-text">${item.grouptour_sketch}</p>
		                        </div>
		                    </div>
		                    <div class="list-content">
		                        <div style="overflow: hidden;height:50px;">
		                            <span class="list-info-text">${item.grouptour_name}</span>
		                            <c:forEach items="${fn:split(item.grouptour_tab,',')}" var="lable">
		                                <span class="list-info-date">${lable}</span>
		                            </c:forEach>
		                        </div>
		                        <div style="overflow: hidden;height:54px;margin-top:12px;" class="list-text-detail">
		                           ${item.grouptour_sketch}
		                        </div>
		                        <p>
		                            <span class="list-info-amount">￥${item.grouptour_price}</span>
...
var imgPrefix = '${SETTINGPD.IMAGEPATH}';
   $(function(){
	   ...
	   $(".sale-row").children().click(function(){
		   ...
		   var url = "/travel/route?currentPage=1&order_type="+data_code;
		   window.location.href=url;
	   });
	   requestContent();
   });
   var order_type = "${pd.order_type}";
   //重写nextPage方法
   function nextPage(num){
	   var url = "/travel/route?currentPage="+num+"&order_type="+order_type;
	   window.location.href=url;
   }

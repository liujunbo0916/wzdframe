	...
	<body>
		<div class="wrap-box">
			<div class="content">
				<div class="inpt">
					<label>收件人姓名</label>
					<input type="text" id="contact_name" name="contact_name" value="${address.contact_name}"/>
					<input name="address_id" type="hidden" value="${address.address_id}" id="address_id" />
				</div>
				<div class="inpt">
					<label>手机</label>
					<input type="number" name="contact_phone" id="contact_phone" value="${address.contact_phone}" />
				</div>
					<div class="inpt content-block">
					<label>省市区</label>
					<label id="demo2" style="width:60%"><c:if test="${empty address.province}">&nbsp;&nbsp;</c:if><c:if test="${not empty address.province}">${address.province}<c:if test="${not empty address.city}">,${address.city}</c:if><c:if test="${not empty address.area}">,${address.area}</c:if></c:if></label>
					 <%-- <input id="demo2" type="text"  readonly=""    value="${address.province}<c:if test="${not empty address.city}">,${address.city}</c:if><c:if test="${not empty address.area}">,${address.area}</c:if>" /> --%>  
					<input id="value2" type="hidden"  value="${address.province_id},${address.city_id},${address.area_id}" />
				</div>
				<div class="inpt ">
					<label>详细地址</label>
		...
		<div class="wrap text-center fred" style="background-color: #ff7d13;color: #FFF;" onclick="<c:if test="${empty address.address_id}">Address.addAddress();</c:if>
		               <c:if test="${not empty address.address_id}">Address.editAddress();</c:if>"
		               >保存该地址</div>
		...
		<script>
			...
			Address.data.provs_data = JSON.parse('${province}');
			Address.data.citys_data = JSON.parse('${city}');
			Address.data.dists_data = JSON.parse('${dist}');
			var area2 = new LArea();
			area2.init({
				'trigger': '#demo2',
				'valueTo': '#value2',
				...
				'type': 2,
				'data': [Address.data.provs_data, Address.data.citys_data, Address.data.dists_data]
			});
		</script>
	</body>
</html>
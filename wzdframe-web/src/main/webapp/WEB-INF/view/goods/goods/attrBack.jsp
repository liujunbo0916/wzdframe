								...
								<c:forEach items="${varList}" var="attr" varStatus="status">
									<div class="form-group">
										<input type="hidden"
											...
											type="hidden" name="goodsAttr[${status.index}].attr_id"
										...
										<!-- 隐藏域用作入库 -->
										<input type="hidden"
											name="goodsAttr[${status.index}].attr_name" id="attr_name"
											class="form-control" value="${attr.attr_name}" required /> <input
											type="hidden" name="goodsAttr[${status.index}].attr_sort"
											id="attr_sort" class="form-control" value="${attr.attr_sort}"
											required /> <input type="hidden"
											name="goodsAttr[${status.index}].is_sale" id="is_sale"
											class="form-control" value="${attr.is_sale}" required />
										<div class="col-sm-8">
											<!-- （1单行文本，2多行文本，3单选，4多选） -->
											...
											<c:if test="${attr.attr_input == 3 }">
												<select name="goodsAttr[${status.index}].attr_value"
													id="attr_value" class="form-control" style="width: 100px;">
											...
											<c:if test="${attr.attr_input == 4 }">
												...
												<c:forEach var="val" varStatus="vs"
													items="${fn:split(attr.attr_values,',')}">
													<c:forEach items="${fn:split(attr.attr_value,',')}"
														var="tempItem">
														<c:if test="${fn:trim(tempItem) eq fn:trim(val)}">
															<c:set value="${fn:trim(tempItem)}" var="tempAttr" />
														</c:if>
													</c:forEach>

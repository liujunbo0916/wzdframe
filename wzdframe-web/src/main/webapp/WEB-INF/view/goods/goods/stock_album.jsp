...
<body class="gray-bg">
						...
						<div class="lightBoxGallery">
							<c:forEach items="${albumModel}" var="item">
									<div style="float:left;width: 170px; height: 136px;margin-left:40px;">
										<a  href="${SETTINGPD.IMAGEPATH}${item}" style="position: relative;" 
											data-gallery=""> <img class="album_p" style="width: 160px; height: 106px;"
											src="${SETTINGPD.IMAGEPATH}${item}">
										</a>
										<span onclick="delStockImg('${item}');" class="delImg">移除</span>
									</div>
							</c:forEach>
	...
	<script type="text/javascript">
	  var goods_id = '${pd.sku_id}';
	  var imgNum = '${fn:length(albumModel)}';
	  var serverUrl = "/sys/goods/goods/saveStockImg";
	</script>
	<!-- basic scripts -->
	<!-- 页面底部js¨ -->
	...
	$("#imgNum").text(5-imgNum);
	  ...
	  function closed(){
		  parent.callBackChildPage('${pd.sku_id}');
		  parent.layer.close(index);
	   }
	  function delStockImg(imgPath){
		  bootbox.confirm("确定删除该图片？",function(result){
			  if(result){
				  Ajax.request("/sys/goods/goods/delStockImg",{"data":{"imgPath":imgPath,"sku_id":'${pd.sku_id}'},"success":function(data){
					  if(data.result == 200){
						  location.reload();
					  }else{

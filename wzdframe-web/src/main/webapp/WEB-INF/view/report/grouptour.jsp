			...
			<div class="col-sm-12">
				<div class="ibox">
					...
					<div class="ibox-content">
						<form action="/sys/report/grouptour/list" method="post"
							name="Form" id="Form">
							<div class="row">
								...
								<div class="project-list">
									<table id="simple-table"
										...
										<tbody>
											<!-- 开始循环 -->
											<c:choose>
												<c:when test="${not empty grouptourList}">
													<c:forEach items="${grouptourList}" var="item">
														<tr>
															<td>${item.orderSn}</td>
															<td>${item.totalPrice}</td>
															<td>${item.payPrice}</td>
															<td>
																<p>下单人姓名：${item.linkPerple}</p>
																<p>下单人手机号：${item.linkPhone}</p>
																<p>跟团游名称：${item.grouptourName}</p>
																<p>跟团游服务电话：${item.grouptourPhone}</p>
															</td>
															<td>已使用</td>
